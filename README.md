...
最近一版 **v1.4.5** —— 消耗估算口径三处修正：**峰谷改按「事件发生时刻」计价**（旧实现按「打开面板那一刻」，谷时跑的量在峰时查看会被翻倍）、**法定节假日全天算谷时**、**缓存写入按官方 ×125% 单独计价**；同时把八家厂商的价格表逐条对官方原文重核，并补录 20+ 条新模型（GPT-6 Astra、Claude Fable 5.1、GLM-5.3-FlashX、MiniMax-M3 等）。一版一行的完整历史见 [`CHANGELOG.md`](CHANGELOG.md)。
