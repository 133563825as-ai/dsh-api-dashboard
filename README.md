...
> 🧪 **你现在看的是预发行分支 `preview/desktop`** —— 版本 `v1.5.0-desktop-preview.7`（补上宽屏触屏档的宿主侧栏竖线抑制，待平板实测）。
...
最近一版 **v1.4.2**（本分支为预发行 `v1.5.0-desktop-preview.7`，宽屏触屏档的宿主侧栏竖线抑制，**待平板验证**）—— DeepSeek 多币种钱包读取修复 + 桌面/平板适配预览。一版一行的完整历史见 [`CHANGELOG.md`](CHANGELOG.md)。
