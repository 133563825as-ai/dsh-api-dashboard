...
- **v1.4.3** — 修复**子代理消耗显示**：分叉出来的子代理会把**父会话的历史消耗**算进自己那一行（实测看到「只用了 DeepSeek 的子代理也挂着一笔美元」），而且原实现还会把它的**后代消耗**一并汇总进来。现在子代理胶囊**只显示它自己新产生的消耗**：继承历史、兄弟子代理、后代子代理都不再计入。**新开（非分叉）的子代理行为完全不变**（它没有继承历史，自身即全部）；主会话自己的统计口径也一字未改。拿不到自身用量边界时显示「等待」而不是回退成错的数字（状态结构版本 2 → 3，旧缓存不凑合显示）
